...
* If Context-7 MCP is installed, ground library comparisons (benchmark scores, documentation coverage, active version support) using Context-7 MCP (`resolve-library-id` $\to$ `query-docs`) before formalizing the ADR. If not installed, suggest installing it (`agy mcp add context7 https://mcp.context7.com/mcp`) for authoritative metrics.
